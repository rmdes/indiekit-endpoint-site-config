...
import { migrateV3toV4 } from "../storage/migrate-v3-to-v4.js";
...
export function adminApiRouter(Indiekit) {
   ...
   *
   * Responds `{ docs, report, seeded }` where:
   ...
   *   - seeded  — CURRENT compositions-collection state: which of the (at
   *               most three) migrator-computed surface ids already exist.
   *               Distinct from report.seeded ("what this run wrote" —
  ...
  router.get("/migration-preview", async (req, res, next) => {
    try {
      ...
      // The migrator computes at most these three surface ids — findOne each
      // rather than widening the db surface with a find() requirement.
      const compositions = db.collection("compositions");
      const seeded = [];
      for (const _id of ["homepage", "collection:default", "posttype:default"]) {
        const existing = await compositions.findOne({ _id });
        if (existing) seeded.push(_id);
      }

      res.setHeader("Cache-Control", "no-store");
      res.json({ docs, report, seeded });
    } catch (error) {
      next(error);
    }
  });
