...
import { migrateV3toV4, MIGRATION_SURFACE_IDS } from "../storage/migrate-v3-to-v4.js";
...
export function adminApiRouter(Indiekit) {
  ...
  // Phase 7d — the /api/sections + /api/widgets legacy discovery endpoints were
  // removed with the legacy three-getter adapter (no consumer: the v3 admin UI
  // is retired; the theme reads block-catalog.json). The v4 composition editor
  // uses the block catalog directly.

  /**
   * GET /site-config/api/migration-preview
   *
   * Admin-only diagnostic for the Phase 2 dual-running v3 → v4 migration.
   * Recomputes the migration FRESH on every request with `dryRun: true` —
   * this endpoint NEVER writes (the boot-time init in index.js is the single
   * sanctioned non-dry caller, per the migrator's TOCTOU precondition).
   *
   * Responds `{ docs, report, existing }` where:
   *   - docs     — the v4 composition docs the migrator would seed
   *   - report   — uniform migrator report (skipped/valid/errors/warnings/
   *                seeded/skippedExisting; dry-run, so the last two are empty)
   *   - existing — CURRENT compositions-collection state: which of the
   *                migrator-computed surface ids (MIGRATION_SURFACE_IDS)
   *                already exist. Deliberately NOT named `seeded` —
   *                report.seeded means "what this run wrote" (nothing, in a
   *                dry run); this field has skippedExisting semantics.
   *
   * 503 when no database is configured (same db-handle guard idiom as the
   * storage modules). Catalog is read at request time from
   * `Indiekit.config.application.blockCatalog` (populated by scanPlugins),
   * per the established controller convention.
   */
  router.get("/migration-preview", async (req, res, next) => {
    try {
      ...
      if (!db) {
        ...
        return res
          .status(503)
          .json({ error: "Migration preview requires a database (MongoDB is not configured)" });
      }

      const catalog = Indiekit.config?.application?.blockCatalog || [];
      const { docs, report } = await migrateV3toV4(db, catalog, { dryRun: true });

      // The migrator computes at most MIGRATION_SURFACE_IDS — findOne each
      // rather than widening the db surface with a find() requirement.
      const compositions = db.collection("compositions");
      const existing = [];
      for (const _id of MIGRATION_SURFACE_IDS) {
        const doc = await compositions.findOne({ _id });
        if (doc) existing.push(_id);
      }

      res.setHeader("Cache-Control", "no-store");
      res.json({ docs, report, existing });
    } catch (error) {
      next(error);
    }
  });
