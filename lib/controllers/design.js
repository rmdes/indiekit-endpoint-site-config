...
export function groupAvailableBlocks(catalog, names, options = {}) {
  ...
  for (const entry of catalog) {
    ...
    groups.get(group).push({
      ...
      regions: arrangement === "stack" ? regions.filter((r) => r !== "sidebar") : regions,
    });
  }
...
function decorateZones(zones, catalog, names, mode, zoneModel) {
  ...
  return {
    hero: zones.hero ? card(zones.hero, "hero") : null,
    main: zones.main.map((node) => card(node, "main")),
    sidebar: zones.sidebar.map((node) => card(node, "sidebar")),
    footer: zones.footer.map((node) => card(node, "footer")),
  };
}
...
export function designRouter(Indiekit, overrides = {}) {
  ...
  function resolveSurface(request, response, next) {
    const entry = getSurface(request.params.surface);
  ...
  router.get("/", async (request, response, next) => {
    try {
      ...
      if (!db) return;
      // The single live surface (homepage). Its card carries draft/updatedAt
      // metadata from the compositions doc; the rest are static disabled
      // placeholders until their phase registers them (6.3/6.4/6.5).
      const homeEntry = getSurface("homepage");
      const doc = await db.collection("compositions").findOne({ _id: homeEntry.surfaceId });
      const surfaces = [
        {
          key: homeEntry.hubKey,
          href: `/site-config/design/${homeEntry.routeKey}`,
          enabled: true,
          exists: Boolean(doc),
  ...
  surface.post("/blocks/add", async (request, response, next) => {
    try {
      ...
      if (blockEntry.multiple === false && typePresent(loaded.zones, parsed.type)) {
  ...
  surface.post("/blocks/restore", async (request, response, next) => {
    try {
      ...
      if (blockEntry.multiple === false && typePresent(loaded.zones, payload.node.type)) {
        return flashError(response, entry, "duplicate");
      }
      if (findNode(loaded.zones, payload.node.id)) return flashError(response, entry, "duplicate");
