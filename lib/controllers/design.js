...
function blockCard(node, catalogMap, names, mode, zone) {
  ...
  return {
    ...
    // empty when the block has nowhere else to go (or is unknown).
    legalZones: entry
      ? Object.keys(ZONE_TO_REGION).filter((z) => z !== zone && placementAllows(entry, z))
      : [],
...
function decorateZones(zones, catalog, names, mode) {
  ...
  return {
    hero: zones.hero ? blockCard(zones.hero, map, names, mode, "hero") : null,
    main: zones.main.map((node) => blockCard(node, map, names, mode, "main")),
    sidebar: zones.sidebar.map((node) => blockCard(node, map, names, mode, "sidebar")),
    footer: zones.footer.map((node) => blockCard(node, map, names, mode, "footer")),
  };
}
