...
function blockCard(node, catalogMap, names, mode, zone, arrangement) {
  ...
  return {
    ...
    legalZones: entry
      ? Object.keys(ZONE_TO_REGION).filter(
          (z) =>
            z !== zone &&
            !(z === "sidebar" && arrangement === "stack") &&
            placementAllows(entry, z),
        )
      : [],
...
function decorateZones(zones, catalog, names, mode) {
  ...
  const card = (node, zone) => blockCard(node, map, names, mode, zone, zones.arrangement);
  return {
...
export function designRouter(Indiekit, overrides = {}) {
  ...
  async function editorLocals(state, query, extra = {}) {
    ...
    return {
      ...
      availableBlocks: groupAvailableBlocks(catalog, names, { arrangement: zones.arrangement }),
  ...
  router.post("/homepage/blocks/add", async (request, response, next) => {
    try {
      ...
      if (parsed.zone === "sidebar" && loaded.zones.arrangement === "stack") {
        return flashError(response, "invalid-zone");
      }
  ...
  router.post("/homepage/publish", async (request, response, next) => {
    try {
      ...
      if (result.ok) {
        ...
        return response.redirect(303, `${HOME}?published=${now()}`);
      }
