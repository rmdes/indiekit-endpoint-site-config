...
export function designRouter(Indiekit, overrides = {}) {
  ...
  const homeOf = (surface) => `/site-config/design/${surface.routeKey}`;
  ...
  async function editorLocals(surface, state, query, extra = {}) {
    ...
    const base = {
      ...
      // Live-preview is a per-surface capability (6.3 #31 stopgap): the view
      // renders the preview pane + Structural/Preview toggle only when the
      // surface owns the (single, shared) preview slot. Homepage true; a
      // sidebar-only surface (listing) omits it → falsy → notice instead.
      supportsLivePreview: Boolean(surface.supportsLivePreview),
  ...
  /** Resolver middleware: 404 unknown surfaces, attach the entry. */
  ...
  surface.post("/preview", async (request, response, next) => {
    try {
      ...
      if (!db) return;
      const { surface: entry } = request;
      // Live-preview capability gate (6.3 #31 stopgap): the preview is a SINGLE
      // shared slot (preview-draft.json + one token on the siteConfig
      // singleton). A surface that does NOT own it must 404 here BEFORE writing
      // the draft — otherwise the listing editor's preview POST would overwrite
      // (corrupt) the homepage preview slot. Homepage declares the capability →
      // unchanged.
      if (!entry.supportsLivePreview) {
        return response.status(404).send("No live preview for this surface");
      }
      const state = await getEditorState(db, entry.surfaceId);
  ...
  surface.post("/publish", async (request, response, next) => {
    try {
      ...
      if (result.ok) {
        ...
        //
        // Live-preview capability gate (6.3 #31 stopgap): the preview is a
        // SINGLE shared slot (preview-draft.json + one token on the siteConfig
        // singleton). Only the surface that OWNS it (homepage) may rotate/write
        // it — otherwise a listing publish would rotate the shared token and
        // overwrite the homepage preview-draft with the sidebar-only listing
        // tree, corrupting the homepage live-preview and invalidating open
        // homepage /preview/<token>/ URLs. The publish (db promote +
        // writeCompositionJson + redirect) is surface-correct regardless; only
        // the shared preview slot is gated. Homepage → unchanged (still rotates).
        if (entry.supportsLivePreview) {
          try {
