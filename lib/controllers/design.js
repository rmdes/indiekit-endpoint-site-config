 ...
 * restore payload — re-validates strictly (section shape, catalog type,
 * config via validateConfigAgainstSchema, zone placement) and REBUILDS the
 * node from validated parts before it touches the zones.
 *
  ...
  router.post("/homepage/blocks/restore", async (request, response, next) => {
    try {
      ...
      const node = {
        ...
        v: Number.isInteger(payload.node.v) ? payload.node.v : 0,
