...
import {
  ...
  describeChanges,
  matchesPublishEpoch,
} from "../storage/composition-history.js";
import {
  getPreviewState,
  ensureToken,
  bumpRevision,
...
const SUCCESS_FLAGS = ["saved", "added", "moved", "restored", "restoredHistory", "recipe", "published", "discarded", "arranged", "created", "deleted"];
...
// Never-started detection: the Eleventy watcher can miss a publish entirely
// (no build ever starts), leaving the status file at a STALE terminal state
// (ok/failed with finishedAt ≤ the publish epoch) or unknown while the strip
// shows the generic "Rebuilding…" copy forever — isStuckBuild only fires on
// state "building", so a never-started build is invisible to it. Past this
// long after the publish with no building state and no POST-publish terminal
// state observed, the build is presumed missed and the UI offers a republish.
// Mirrored in assets/editor.js (NOT_STARTED_AFTER_MS there).
const NOT_STARTED_AFTER_MS = 90_000;
// Plausibility floor for a publish epoch (same as editor.js's
// MIN_PUBLISH_EPOCH_MS): anything below (legacy "1", garbage, tampered
// values) is not a publish time. 1e12 ms ≈ 2001-09-09.
const MIN_PUBLISH_EPOCH_MS = 1e12;

/**
 * Did the Eleventy watcher miss this publish entirely (no build ever
 * started)? True only with a plausible publish epoch, past the 90s grace
 * window, when the status is neither "building" nor a FRESH terminal state
 * (ok/failed whose finishedAt parses and postdates the publish — that build
 * ran). Tolerant like isStuckBuild: a null/garbage status counts as
 * not-started once past the threshold (an absent file after a publish is
 * exactly the missed-publish symptom).
 *
 * @param {object | null | undefined} status Parsed build-status content
 * @param {number} publishedEpochMs Publish time (epoch ms, pre-validated)
 * @param {number} nowMs Current epoch ms (injected for tests)
 * @returns {boolean}
 */
export function isNeverStartedBuild(status, publishedEpochMs, nowMs) {
  if (typeof publishedEpochMs !== "number" || !(publishedEpochMs >= MIN_PUBLISH_EPOCH_MS)) {
    return false;
  }
  if (!(nowMs - publishedEpochMs > NOT_STARTED_AFTER_MS)) return false;
  const state = status?.state;
  if (state === "building") return false; // stuck detection owns overdue builds
  if (state === "ok" || state === "failed") {
    const finishedAt =
      typeof status.finishedAt === "string" ? Date.parse(status.finishedAt) : Number.NaN;
    if (finishedAt > publishedEpochMs) return false; // fresh terminal — the build RAN
  }
  return true;
}
...
export function readFlash(query = {}) {
  ...
  // SECURITY: any flash value the view CONCATENATES INTO AN i18n KEY must be
  // allowlisted here. On an unknown key, i18n's missing-key fallback echoes
  // the whole key — raw query payload included — through the catalogs'
  // unescaping {{{…}}} placeholders into the |safe notification banner
  // (reflected XSS). Closed enums / key-charset checks make that echo path
  // unreachable. (Escaped {{label}} interpolations like `removed` are fine.)
  if (typeof query.error === "string" && /^[a-z0-9-]{1,64}$/.test(query.error)) {
    flash.error = query.error;
  }
  if (query.noUndo) flash.undoUnavailable = true;
  if (typeof query.sidebarMoved === "string" && /^\d{1,4}$/.test(query.sidebarMoved)) {
    flash.sidebarMoved = query.sidebarMoved;
  }
  // The restore-from-history flash carries the restored entry's structured
  // label (month number + change noun) so the view can render the localized
  // "Restored 'The July rearrangement' …" line at display time.
  if (typeof query.hm === "string" && /^([1-9]|1[0-2])$/.test(query.hm)) {
    flash.historyMonth = query.hm;
  }
  if (typeof query.hn === "string" && HISTORY_NOUNS.has(query.hn)) {
    flash.historyNoun = query.hn;
  }
  return flash;
}
...
export function designRouter(Indiekit, overrides = {}) {
  ...
  async function editorLocals(surface, state, query, extra = {}) {
    ...
    return {
      ...
      // Publish-time history ring (newest first); empty → the view omits the
      // History section entirely.
      history: Array.isArray(state.doc.history) ? state.doc.history : [],
  ...
  router.get("/", async (request, response, next) => {
    try {
      ...
      for (const key of HUB_CARD_ORDER) {
        ...
        // card lists EVERY page:<slug> composition (T5). Each page carries its
        // slug/route/title/draft state plus a slug-scoped Edit link (T1 routing)
        // and a Delete action (T3 unpublish route); the card also exposes a
        // create affordance (POST /pages). The singleton card model can't
        // represent N pages, so this branch builds a distinct collection card.
        if (entry.isCollection) {
          const pages = (await listPages(db)).map((page) => ({
            slug: page.slug,
            route: page.route,
            title: page.title,
            hasDraft: page.hasDraft,
            updatedAt: page.updatedAt,
            editHref: `/site-config/design/${entry.routeKey}/${page.slug}`,
            deleteHref: `/site-config/design/${entry.routeKey}/${page.slug}/delete`,
          }));
          surfaces.push({
            key: entry.hubKey,
            enabled: true,
            isCollection: true,
            createHref: `/site-config/design/${entry.routeKey}`,
            pages,
          });
          continue;
        }
      ...
      // Hub flash (6.5): the pages create/delete routes redirect here with
      // ?error=<code> / ?deleted=1 (they can't land on the bare /pages 404).
      // readFlash maps those query params to {error}/{success}; the view
      // renders a banner reusing the SHARED errors map (siteConfig.design.errors)
      // and the flash map (siteConfig.design.flash) the editor already uses.
      const flash = readFlash(request.query);
      response.render(HUB_VIEW, { activeTab: "design", surfaces, flash });
    } catch (error) {
      next(error);
    }
  });
  ...
  surface.get("/", async (request, response, next) => {
    try {
      ...
      if (query.published) {
        ...
        const nowMs = now();
        // SECURITY: per the readFlash allowlist discipline, the raw query
        // value must be validated before any use — a strict 13-16 digit epoch
        // shape gates parseInt; anything else (legacy ?published=1, garbage)
        // disables the never-started computation and never reaches numbers.
        const epoch =
          typeof query.published === "string" && /^\d{13,16}$/.test(query.published)
            ? Number.parseInt(query.published, 10)
            : null;
        extra = {
          buildStatus: mergeBuildStatus(status, nowMs),
          // No-JS mirror of editor.js's never-started branch: the strip
          // renders the missed-publish copy + a visible republish form.
          buildNotStarted: epoch !== null && isNeverStartedBuild(status, epoch, nowMs),
        };
        // "Tell your readers?" (spec §5.3 wildcard): when THIS publish
        // changed the tree (the history head was archived in the same
        // request that stamped the flash epoch) and the share endpoint is
        // loaded, offer a pre-drafted note built from the reader-visible
        // diff. The view assembles the copy from locale keys and links to
        // the share form prefill — nothing is ever posted automatically.
        const shareEndpoint = Indiekit.config?.application?.shareEndpoint;
        const historyHead = state.doc?.history?.[0];
        if (
          shareEndpoint &&
          epoch !== null &&
          historyHead &&
          // Exact-equality guard (review finding): a real publish stamps
          // history[0].archivedAt and doc.updatedAt from the SAME variable in
          // the SAME atomic $set (publishDraft), while a no-op republish
          // (e.g. the stuck-build "Publish again" hatch) advances updatedAt
          // WITHOUT touching history — so equality proves THIS publish
          // archived the head, and a re-promote can't re-show a stale prompt.
          historyHead.archivedAt === state.doc.updatedAt &&
          matchesPublishEpoch(historyHead.archivedAt, epoch)
        ) {
          const changes = describeChanges(historyHead.tree, state.doc.tree);
          if (changes) {
            const catalog = catalogOf();
            const labelOf = (type) =>
              catalog.find((e) => e.id === type)?.label ?? type;
            extra.tellReaders = {
              added: [...new Set(changes.added)].map(labelOf),
              removed: [...new Set(changes.removed)].map(labelOf),
              rearranged: changes.rearranged,
              shareEndpoint,
              siteUrl: Indiekit.config?.publication?.me || "",
            };
          }
        }
      }
  ...
  // Restore a history entry AS A DRAFT: the archived tree becomes the
  // current draft (via the ordinary saveDraft) for review and an explicit
  // publish — the published tree and the history ring are never touched.
  surface.post("/history/:index/restore", async (request, response, next) => {
    try {
      const db = requireDb(response);
      if (!db) return;
      const { surface: entry } = request;
      // Index gate: a single digit 0–9 (the ring is HISTORY_LIMIT deep) —
      // negatives, non-integers and anything ≥ 10 flash-reject up front.
      const raw = request.params.index;
      const index = /^\d$/.test(raw) ? Number.parseInt(raw, 10) : -1;
      if (index < 0 || index >= HISTORY_LIMIT) {
        return flashError(response, entry, "history-invalid");
      }
      const state = await getEditorState(db, entry.surfaceId);
      if (!state) return flashError(response, entry, "no-composition");
      const archived = state.doc.history?.[index];
      if (!archived?.tree) return flashError(response, entry, "history-invalid");
      const saved = await saveDraft(db, entry.surfaceId, archived.tree);
      if (!saved.ok) return flashError(response, entry, saved.error);
      // The flash carries the entry's structured label (month + noun) so the
      // banner can name what was restored, localized at render time.
      const params = new URLSearchParams({ restoredHistory: "1" });
      if (archived.label) {
        params.set("hm", String(archived.label.month));
        params.set("hn", String(archived.label.noun));
      }
      return response.redirect(303, `${homeOf(entry)}?${params}`);
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post("/pages", async (request, response, next) => {
    try {
      ...
      // reject redirects back to the HUB (/site-config/design/), where the
      // banner renders. NOT /site-config/design/pages — that bare collection
      // route is DESIGNED to 404 (resolveSurface), so a flash sent there lands
      // on a white 404. The hub is the design router's root.
      const hubError = (code) =>
        response.redirect(303, `/site-config/design/?error=${code}`);
  ...
  router.post("/pages/:slug/delete", injectPageSurface, async (request, response, next) => {
    try {
      ...
      // Land on the HUB (?deleted=1), where the banner renders — NOT the bare
      // /pages collection route (which 404s).
      return response.redirect(303, "/site-config/design/?deleted=1");
    } catch (error) {
      next(error);
    }
  });
