...
export function designRouter(Indiekit, overrides = {}) {
  ...
  //
  // T4 SEAM: the pages.json artifact REWRITE on delete is the array writer's
  // job (writePagesJson) — T4 wires it here (rewrite the published-pages array
  // after the doc is removed). T3 only removes the doc; NO .md is generated.
  router.post("/pages/:slug/delete", injectPageSurface, async (request, response, next) => {
    try {
      ...
      // TODO(6.5-T4): await writePagesJson(db, outputDir) here to rewrite
      // content/_data/compositions/pages.json without the removed page.
