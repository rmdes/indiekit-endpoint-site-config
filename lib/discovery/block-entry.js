...
export function validBlockEntry(entry) {
  ...
  if (entry.render?.renderer !== undefined && !GENERIC_RENDERERS.has(entry.render.renderer)) {
    errors.push(`render.renderer must be one of the ${GENERIC_RENDERERS.size} generic renderers`);
  }
    ...
    switch (def.type) {
      case "number":
        properties[name] = { type: "number", ...title,
          ...(typeof def.min === "number" ? { minimum: def.min } : {}),
          ...(typeof def.max === "number" ? { maximum: def.max } : {}) };
        break;
      case "boolean":
