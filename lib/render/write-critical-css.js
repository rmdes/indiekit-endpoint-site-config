 ...
 */
/**
 * Build the CSS `font-family` value for the body, honoring the operator's
 * selected sans font. "system-ui" (or empty) emits the bare system stack so the
 * body{} rule never quotes a font that does not exist.
 *
 * @param {string} [name] - typography.sans value
 * @returns {string} font-family value
 */
function formatFontFamily(name) {
  const n = (name || "").trim();
  if (!n || n === "system-ui") return 'system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif';
  return `"${n}",system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif`;
}

/**
 * The body{} rule, with the font-family resolved from branding typography so
 * first paint matches the operator's chosen sans (no Inter FOUC).
 *
 * @param {object} config - Full site config
 * @returns {string} CSS rule
 */
function bodyFontRule(config) {
  const sans = config?.branding?.typography?.sans;
  return `body{margin:0;font-family:${formatFontFamily(sans)};line-height:1.5;-webkit-font-smoothing:antialiased}`;
}

/**
 * Emit the self-hosted Inter @font-face declarations ONLY when Inter is the
 * selected sans and it is self-hosted. When the operator picks a different sans
 * the theme's own font loading handles it; shipping Inter faces here would be
 * dead weight (and would not match the body font-family anyway).
 *
 * @param {object} config - Full site config
 * @returns {string} CSS @font-face rules (or empty string)
 */
function fontFaceRules(config) {
  const sans = (config?.branding?.typography?.sans || "").trim();
  const hosting = config?.branding?.typography?.hosting;
  if (sans !== "Inter" || hosting !== "self") return "";
  return `@font-face{font-family:'Inter';font-style:normal;font-display:optional;font-weight:400;src:url(/fonts/inter-latin-400-normal.woff2) format('woff2')}@font-face{font-family:'Inter';font-style:normal;font-display:optional;font-weight:600;src:url(/fonts/inter-latin-600-normal.woff2) format('woff2')}@font-face{font-family:'Inter';font-style:normal;font-display:optional;font-weight:700;src:url(/fonts/inter-latin-700-normal.woff2) format('woff2')}`;
}
...
export function renderCriticalCss(config) {
  ...
  if (mode === "light") {
    return [bodyFontRule(config), LAYOUT_SHELL, fontFaceRules(config), emitSet("", light), ""].join("\n");
  }

  if (mode === "dark") {
    return [bodyFontRule(config), LAYOUT_SHELL, fontFaceRules(config), emitSet("", dark), ""].join("\n");
  }
