 ...
 * Strips server-only fields (e.g. updatedBy may contain user email — PII).
 *
...
export function renderSiteJson(config) {
  // Strip server-only fields — updatedBy may contain a user email or profile URL (PII)
  const { updatedBy, ...publicConfig } = config;
