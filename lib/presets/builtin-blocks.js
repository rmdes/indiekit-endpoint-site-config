 ...
 *   from a legacy config would fail migration. Exception: `embed` shipped
 *   AFTER the v3 → v4 migration (no legacy configs exist), and its url host
 *   allowlist is a security gate that `required` must back up.
...
export const BUILTIN_BLOCKS = Object.freeze([
  Object.freeze({
    ...
    multiple: false,
    // Schema is the UNION of both v3 hero shapes: the v3 homepage doc's hero
    // object (showSocial, ctaText, ctaUrl — see DEFAULTS_HOMEPAGE.hero in
    // lib/storage/defaults-homepage.js; `enabled` excluded as a placement
    // concern) and the legacy section configSchema (showAvatar,
    // showSocialLinks). Task 7's migrator pours the doc's hero object into
    // this schema with stripUnknown — a doc key missing here would be
    // silently stripped from every migrated site. ctaText/ctaUrl text
    // defaults intentionally omitted — the doc ("Read more"/"/about/") and
    // hero.njk own the fallback; baking English strings into schema defaults
    // would fight per-site localization.
    schema: Object.freeze({
      ...
      properties: Object.freeze({
        ...
        // Renderer-dead: hero.njk never reads showSocialLinks (it reads
        // showSocial). Kept for legacy-form compat — candidate for removal
        // in Phase 4.
  ...
  Object.freeze({
    ...
    schema: Object.freeze({
      ...
      additionalProperties: false,
      properties: Object.freeze({
        // Legacy section capped maxItems at 20 — widened to the global
        // maxItems ≤ 50 cap (module convention).
  ...
  Object.freeze({
    ...
    schema: Object.freeze({
      ...
      properties: Object.freeze({
        ...
        // zone-preset concern for Phase 4 (see module comment). The widget
        // variant also capped maxItems at 20; the merged entry keeps the
        // section variant's 1–50 bounds (= the global maxItems cap).
        maxItems: Object.freeze({
          type: "integer",
          title: "Max items",
          minimum: 1,
        ...
        // Discovered in the Phase 2 production diff: the live rmendes v3
        // config carries excludeTypes (written by the v3 admin UI, consumed
        // by the theme) but the legacy mini-DSL never declared it — the v4
        // schema is strictly richer than the legacy one here.
        excludeTypes: Object.freeze({
          type: "array",
          title: "Post types to exclude",
          items: Object.freeze({ type: "string" }),
        }),
      }),
    }),
  ...
  Object.freeze({
    ...
    schema: Object.freeze({
      type: "object",
      additionalProperties: false,
      // Legacy empty-string defaults ("" for title/content) intentionally
      // dropped as semantic no-ops — the theme owns the empty fallback.
      properties: Object.freeze({
        title: Object.freeze({
          type: "string",
  ...
  Object.freeze({
    ...
    label: "Embed",
    description:
      "Embedded media player (YouTube, Vimeo, Spotify, SoundCloud, Acast, Bandcamp, CodePen)",
    icon: "video",
    category: "content",
    placement: Object.freeze({
      regions: Object.freeze(["main", "sidebar", "footer"]),
    ...
    schema: Object.freeze({
      type: "object",
      additionalProperties: false,
      // The only builtin schema using required — see the module conventions:
      // no legacy configs exist for embed, and the host allowlist below is a
      // security gate a defaulted/absent url would sidestep.
      ...
      properties: Object.freeze({
        url: Object.freeze({
          ...
          title: "Media URL",
          maxLength: 500,
          "x-allowed-url-hosts": Object.freeze([
            "youtube.com",
            "youtu.be",
    ...
    data: Object.freeze({ source: "config" }),
    render: Object.freeze({ renderer: "embed" }),
  }),
  Object.freeze({
    id: "posting-activity",
    version: 1,
  ...
  // Phase 7d — the 7 plugin-owned widget blocks (github-repos, funkwhale,
  // blogroll, feedland, webmentions, recent-comments, fediverse-follow) were
  // REMOVED from this builtin seed. Their owning plugins now declare them via
  // get blocks() (Phases 7b/7c), so scanPlugins stamps requiresPlugin and the
  // block appears in a site's catalog ONLY when its plugin is loaded — catalog
  // presence is now the gate (the theme's render-time legacy-map/ENDPOINT_SLUGS
  // gate is retired in the same release). Keeping them here as requiresPlugin-
  // null seeds would make them render ungated on plugin-less sites.
  Object.freeze({
