...
import { historyLabel, HISTORY_LIMIT } from "./composition-history.js";
...
export async function publishDraft(db, surfaceId, catalogEntries, options = {}) {
  ...
  const updatedAt = now();
  // History snapshot: when a published tree exists AND the promote actually
  // changes it (historyLabel is null on identical trees), archive the
  // OUTGOING tree at history[0], capped at HISTORY_LIMIT. Built from the
  // same doc read the concurrency filter protects, and written in the SAME
  // atomic $set as the promote — a racing draft misses the filter and
  // neither the promote nor the snapshot lands. First-ever publishes (no
  // prior tree) and no-change republishes record nothing; boot seed/reseed
  // paths write docs directly (never through here) so they never snapshot.
  const label = doc.tree === undefined ? null : historyLabel(doc.tree, candidate, updatedAt);
  const history = label
    ? [
        { tree: doc.tree, label, archivedAt: updatedAt },
  ...
  // and history fields stripped (writeCompositionJson whitelists anyway —
  // this keeps injected writers honest too). Errors propagate (caller catches).
  const {
    draftTree: _draftTree,
    draftUpdatedAt: _draftUpdatedAt,
    history: _history,
    ...published
  } = doc;
