...
/**
 * Unquote a YAML scalar to its logical value, matching what gray-matter (and thus
 * the census) produces. Handles single-quoted ('' → ') and double-quoted (JSON
 * escapes) scalars; bare scalars pass through. Needed because malformed imports
 * can store quoted values like `'["RSS"]'` whose logical value (`["RSS"]`) is what
 * the rename map keys on.
 * @param {string} raw
 * @returns {string}
 */
...
/**
 * Quote a value as a YAML scalar only if it needs it (special chars, leading/
 * trailing space, empty, or a reserved word). Canonical category names are
 * simple, so this is usually a no-op.
 * @param {string} v
 * @returns {string}
 */
...
export function rewriteCategoryFrontmatter(content, renameMap) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inline) {
      const logical = unquoteYamlScalar(inline[1]);
      const to = renameMap[logical];
      if (to !== undefined && to !== logical) {
        out.push(`category: ${quoteIfNeeded(to)}`);
        changed = true;
      } else {
        out.push(line);
      }
      ...
      while (j < lines.length) {
        ...
        if (!m) break;
        const logical = unquoteYamlScalar(m[2]);
        const to = renameMap[logical];
        const renamed = to !== undefined && to !== logical;
        if (renamed) changed = true;
        items.push({ indent: m[1], raw: m[2], logical: renamed ? to : logical, renamed });
        j++;
      }
      // De-dupe (a merge can collapse two casings into one). Unchanged items keep
      // their original raw text byte-for-byte; renamed items are written fresh.
      const seen = new Set();
      for (const it of items) {
        if (seen.has(it.logical)) {
          changed = true;
          continue;
        }
        seen.add(it.logical);
        out.push(`${it.indent}${it.renamed ? quoteIfNeeded(it.logical) : it.raw}`);
      }
