...
/**
 * True when `value` looks like a zone-model (carries `recognize`/`build`),
 * false when it looks like an options bag (or anything else). Used by the
 * `zonesToTree` wrapper to disambiguate its overloaded 2nd argument.
 * @param {unknown} value
 * @returns {boolean}
 */
const isZoneModel = (value) =>
  Boolean(value) &&
  typeof value === "object" &&
  typeof value.recognize === "function" &&
