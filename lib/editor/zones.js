/**
 * Zones mapping — the Phase 4 editor's constrained view over the recursive
 * v4 composition tree. The editor never edits the tree directly: it edits a
 * flat `{ arrangement, hero, main, sidebar, footer }` shape, and this module
 * maps between that shape and the EXACT tree shapes the migrator/refresh
 * pipeline produces (lib/storage/migrate-v3-to-v4.js `buildHomepageTree`).
 *
 * STRICTNESS IS THE CONTRACT: `treeToZones` recognizes precisely the
 * migrator's shapes and nothing else. Anything off-shape returns
 * `{ custom: true, tree }` — the editor renders a read-only notice instead.
 * The failure mode of wrongly-custom is a read-only editor; the failure mode
 * of wrongly-recognized is silent data corruption on the next save (the
 * rebuild would drop whatever the recognizer glossed over). When in doubt,
 * custom. Never throws, never lossy-coerces.
 *
...
// zonesToTree rebuilds containers from scratch, so any unrecognized key
 ...
 */
export function treeToZones(tree) {
 ...
 * `treeToZones` — for any recognized tree,
 * `zonesToTree(treeToZones(t)) deep-equals t` INCLUDING ids: container ids
 ...
 */
export function zonesToTree(zones, options = {}) {
