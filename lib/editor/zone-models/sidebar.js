/**
 * Sidebar zone-model — the shared SIDEBAR-ONLY surface shape (Phase 6.3+).
 *
 * A zone-model is the per-surface contract for the constrained editor: it maps
 * a v4 composition tree ↔ the editor's flat zones shape. This model is SHARED
 * by every sidebar-only surface: the `collection:default` listing surface
 * (Phase 6.3) and the `posttype:default` post surface (Phase 6.4). Both edit a
 * composition the migrator seeds as a single complementary (sidebar) zone — no
 * hero/main/footer, no arrangement:
 *
 *   stack(root, [stack(complementary, <sections>, { sticky: true })])
 *
 * (migrate-v3-to-v4.js `sidebarComposition` :150-163 → `stack` :71-78).
 *
 * The logic is SHAPE-based, not surface-specific: it recognizes/rebuilds the
 * sidebar-only tree regardless of which surface owns the composition.
 *
 ...
 * @module editor/zone-models/sidebar
 */
...
/**
 * The shared sidebar-only zone-model (used by the `collection:default` listing
 * surface and the `posttype:default` post surface). `recognize`/`build` are
 * `this`-free standalone functions, so they can be referenced directly or
 * destructured without binding. `zoneModel: true` is the explicit discriminant
 * the zones.js overload guard routes on.
 */
export const sidebarZoneModel = {
  zoneModel: true,
  zones: ["sidebar"],
...
/**
 * Back-compat alias for the pre-6.4 name. The listing surface (and its tests)
 * historically referenced `listingZoneModel`; it is now one of several
 * sidebar-only surfaces sharing `sidebarZoneModel`. Same object — the listing
 * surface's model identity is UNCHANGED.
 * @deprecated Prefer `sidebarZoneModel`.
 */
export const listingZoneModel = sidebarZoneModel;
