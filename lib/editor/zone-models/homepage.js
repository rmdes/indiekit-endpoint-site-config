...
// The migrator's 2-1 columns variant token, replicated EXACTLY
// (buildHomepageTree). Homepage-only: the listing surface is sidebar-only and
// has no columns container, so this stays local to the homepage model.
...
export const homepageZoneModel = {
  // Explicit zone-model discriminant: the zones.js overload guard routes on
  // this sentinel (NOT on duck-typed method names), so an options bag that
  // happens to carry build/recognize keys is never misclassified as a model.
  // Every future per-surface model MUST set `zoneModel: true`.
  zoneModel: true,
