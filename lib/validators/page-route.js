 ...
 * @param {object | null | undefined} Indiekit Host — `config.publication.me`
 *   is the site base URL used to derive the FULLY-QUALIFIED url collision leg
 *   (see below). Absent/missing `me` ⇒ the qualified leg is skipped (the
 *   relative legs still run); it never crashes the create flow.
 * @param {{ slug: string, route: string }} target Output of validatePageRoute
 * @returns {Promise<{ ok: true } | { ok: false, error: string }>}
 */
export async function checkRouteCollision(db, Indiekit, { slug, route }) {
  if (!db || typeof db.collection !== "function") return { ok: true };

  // SECURITY (MEDIUM): @indiekit/util getCanonicalUrl stores posts'
  // `properties.url` as a FULLY-QUALIFIED absolute URL (e.g.
  // https://rmendes.net/about/), NOT a relative form. Checking only the
  // relative shapes would miss a real page-post and let a page shadow it. So
  // we add a 4th query form: the absolute URL via `new URL(route, me).href`.
  // Fail-OPEN on this leg ONLY: if `me` is missing (or new URL throws), skip
  // the absolute leg and keep the relative legs — never crash the create flow.
  const urlForms = [route, `/${slug}`, slug];
  const me = Indiekit?.config?.publication?.me;
  if (typeof me === "string" && me !== "") {
    try {
      urlForms.push(new URL(route, me).href);
    } catch {
      // Malformed `me` — skip just the absolute leg, keep the relative legs.
    }
  }

  // (a) posts collection: a published post already owning this url. Match the
  // relative shapes AND the fully-qualified canonical url; an OR keeps us
  // robust to how a given post stores its url.
  try {
    const posts = db.collection("posts");
    if (posts && typeof posts.findOne === "function") {
      const hit = await posts.findOne({
        $or: urlForms.map((url) => ({ "properties.url": url })),
      });
      if (hit) {
