 ...
 * not a transformer, for the migrator). `value` is populated even when
 * `ok: false` — callers MUST check `ok` before persisting it.
 *
...
export function validateComposition(doc, catalogEntries, options = {}) {
  ...
  if (doc.schemaVersion !== 4) {
    errors.push(`schemaVersion must be 4 (got ${doc.schemaVersion})`);
  }
  ...
  if ((typeCounts.get("custom-html") || 0) > MAX_CUSTOM_HTML) {
    errors.push(`more than ${MAX_CUSTOM_HTML} custom-html blocks`);
  }
