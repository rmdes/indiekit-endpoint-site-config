...
/**
 * Check a non-empty string config value against a property's
 * x-allowed-url-hosts allowlist: must parse as a URL, use https, and have a
 * hostname that IS an allowlisted host or a dot-anchored subdomain of one
 * (evilyoutube.com must never match youtube.com). Empty/absent values are the
 * `required` array's job, not this check's.
 * @param {string} label Error-message prefix (e.g. `"url"`)
 * @param {string} value Non-empty string config value
 * @param {string[]} hosts Allowlisted lowercase hosts
 * @returns {string | null} Error message, or null when the value passes
 */
function allowedUrlHostError(label, value, hosts) {
  let url;
  try {
    url = new URL(value);
  } catch {
  ...
  return allowed
    ? null
    : `${label} host "${hostname}" is not in the allowed host list`;
}
...
export function validateSchemaDefinition(schema) {
  ...
  for (const [name, def] of Object.entries(properties)) {
    ...
    if ("x-allowed-url-hosts" in def) {
      const hosts = def["x-allowed-url-hosts"];
      if (def.type !== "string") {
        errors.push(
          `property "${name}": x-allowed-url-hosts only allowed on strings`,
        );
      }
      if (
        !Array.isArray(hosts) ||
        ...
        errors.push(
          `property "${name}": x-allowed-url-hosts must be a non-empty array of lowercase hosts (no protocol, no slash)`,
        );
      }
    }
...
export function validateConfigAgainstSchema(config, schema, options = {}) {
  ...
  for (const [key, raw] of Object.entries(source)) {
    ...
    // Host allowlist only bites on non-empty strings — empty/absent values
    // are the `required` array's job (see allowedUrlHostError).
    if ("x-allowed-url-hosts" in def && typeof raw === "string" && raw !== "") {
      const hostError = allowedUrlHostError(
        `"${key}"`,
        raw,
        def["x-allowed-url-hosts"],
      );
      if (hostError) {
