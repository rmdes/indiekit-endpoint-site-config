...
test("sidebarZoneModel exposes zoneModel:true, sidebar-only zones, and region map", () => {
  assert.equal(sidebarZoneModel.zoneModel, true);
  assert.deepEqual(sidebarZoneModel.zones, ["sidebar"]);
  ...
  assert.deepEqual(sidebarZoneModel.regionMap, { sidebar: "sidebar" });
  assert.equal(typeof sidebarZoneModel.recognize, "function");
  assert.equal(typeof sidebarZoneModel.build, "function");
});
...
test("recognize maps the seed tree to a sidebar zone (no hero/main/footer/arrangement)", () => {
  ...
  const zones = sidebarZoneModel.recognize(tree);
...
test("custom: root with two children (only a single complementary child is on-shape)", () => {
  ...
  assert.equal(sidebarZoneModel.recognize(mangled).custom, true);
});
...
test("custom: root has zero children (no complementary child)", () => {
  ...
  assert.equal(sidebarZoneModel.recognize(tree).custom, true);
});
...
test("recognize never throws on garbage input — returns custom", () => {
  for (const garbage of [null, undefined, 42, "tree", [], {}, { block: "section" },
    ...
    const zones = sidebarZoneModel.recognize(garbage);
...
test("ROUND-TRIP PROPERTY: seed trees with N sidebar sections survive recognize→build deep-equal incl. ids", () => {
  ...
  for (const [index, tree] of sources.entries()) {
    ...
    const rebuilt = sidebarZoneModel.build(zones, { idFactory: makeIds() });
...
test("build mints fresh container ids via idFactory when _containerIds is absent", () => {
  ...
  const tree = sidebarZoneModel.build(zones, { idFactory: makeIds() });
