...
- **AI Transparency:** this plugin owns the `aiTransparency` (bool) + `aiTransparencyUrl` (default `/ai`) feature flags (General tab → `site.features`; defaults in `lib/storage/defaults-site.js`, validated in `lib/controllers/general.js`). They gate the theme's per-post AI-authorship disclosure (`aiTextLevel`/`aiCodeLevel`/`aiTools`/`aiDescription`). Full reference: `documentation-central/docs/2026-07-15-ai-transparency.md`. NOTE: this is a *separate axis* from the crawler-permission `content_signal`/robots policy (`docs/2026-07-01-agent-readable-stack.md`).
